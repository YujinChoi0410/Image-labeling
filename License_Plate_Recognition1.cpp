...
void CHomework::OnClickedLpr()
{
	...
	//입력영상을 136 기준으로 thresholding
	for (y = 0; y < height; y++) {
		for (x = 0; x < width; x++) {
			if (Oimg[y][x] >= 136) Oimg[y][x] = 255; //픽셀값이 136 이상이면 흰색
			else Oimg[y][x] = 0; //그 외에는 검정색
		}
	}
	...
	Image_Labeling(Oimg, width, height, Label, Area, &Num); //image labeling 진행하여 물체 형상 추출하기

	Make_Noise_Removed_Image(Oimg, width, height, Label, Area, Num, 100); //label영상의 noise제거하기
	DisplayCimage2D(Oimg, width, height, x0 + width + 10, y0, FALSE); //noise 제거된 label영상 display하기

	Read_Template(); //templates 읽어오기

	Numeric_Recognition(Oimg, Label, width, height, Area, Num, 100, x0, y0, y0); //숫자 인식해서 해당 정보 display하기
	//Num: 최대 Label 수, Area: 화소가 모여있는 개수
